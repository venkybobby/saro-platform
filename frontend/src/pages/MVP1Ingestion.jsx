...
export default function MVP1Ingestion() {
  ...
  const riskColor = s => s >= 0.7 ? 'var(--accent-red)' : s >= 0.4 ? 'var(--accent-amber)' : 'var(--accent-green)'
  ...
  return (
    <div>
      ...
      <div className="tabs">
        {[['ingest','+ Ingest Document'],['documents','📄 Document Library'],['forecast','📈 Regulatory Forecast']].map(([t,l]) => (
        ...
        <div className="grid-2">
          <div className="card">
            ...
            <div className="form-group">
              ...
              <textarea className="form-textarea" placeholder="Paste regulatory text here — the system will extract entities, identify high-risk clauses, and score compliance risk." value={form.content} onChange={e => setForm(f => ({...f,content:e.target.value}))} />
            </div>
          ...
          <div className="card">
            ...
            {result ? (
                    ...
                    {result.risk_tags.map((t,i) => (
                      <div key={i} style={{ display:'flex',justifyContent:'space-between',alignItems:'center',padding:'6px 0',borderBottom:'1px solid var(--border)' }}>
                        ...
                        <div style={{ display:'flex',alignItems:'center',gap:8,width:140 }}>
                          <div className="progress-bar" style={{ flex:1 }}><div className="progress-fill progress-amber" style={{ width:`${t.probability*100}%` }} /></div>
              ...
              <div className="empty-state"><div className="empty-state-icon">◈</div><div className="empty-state-text">Ingest a document to see analysis results</div></div>
            )}
          </div>
        </div>
      )}
      ...
      {tab === 'documents' && (
        <div className="card">
          <div className="card-header"><span className="card-title">Document Library</span><span className="badge badge-cyan">{docs.length} documents</span></div>
          {docs.length > 0 ? (
            <table className="data-table">
              <thead><tr><th>Title</th><th>Jurisdiction</th><th>Type</th><th>Risk</th><th>Ingested</th></tr></thead>
              <tbody>
                {docs.map((doc,i) => (
                  <tr key={i}>
                    <td style={{ color:'var(--text-primary)',maxWidth:260,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>{doc.title}</td>
                    <td><span className="badge badge-cyan">{doc.jurisdiction}</span></td>
                    <td style={{ color:'var(--text-muted)',fontSize:11 }}>{doc.doc_type}</td>
                    <td><span style={{ color:riskColor(doc.risk_score),fontFamily:'var(--mono)',fontWeight:600 }}>{(doc.risk_score*100).toFixed(0)}%</span></td>
            ...
            <div className="empty-state"><div className="empty-state-icon">📄</div><div className="empty-state-text">No documents yet. Use Ingest to add documents.</div></div>
          )}
        </div>
      )}
              ...
              <div key={i} className="card" style={{ padding:'16px 20px' }}>
                <div style={{ display:'flex',justifyContent:'space-between',alignItems:'flex-start' }}>
                  ...
                  <div style={{ textAlign:'right',marginLeft:20 }}>
                    <div style={{ fontFamily:'var(--mono)',fontSize:24,fontWeight:700,color:'var(--accent-cyan)' }}>{(f.probability*100).toFixed(0)}%</div>
                    <div style={{ fontSize:10,color:'var(--text-muted)' }}>probability</div>
                    <div style={{ fontSize:11,color:'var(--text-muted)',marginTop:4 }}>{new Date(f.predicted_date).toLocaleDateString()}</div>
                  </div>
                </div>
              </div>
            ))}
